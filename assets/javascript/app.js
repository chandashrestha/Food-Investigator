...
function query(restaurant) {
  ...
  var queryURL =
    "https://www.dallasopendata.com/resource/44uy-sq8p.json?program_identifier=" +
    queryRestaurant +
    "&$order=insp_date";
  ...
  .on("child_added", function(childSnapshot) {
    $("#hos").append(
      "<tr>" +
        ...
        "</td>" +
        "<td>" +
        childSnapshot.val().address +
        "</td>" +
        "<td>" +
        childSnapshot.val().results +
        "</td>" +
        "</tr>"
    );
  });
...
function map(restAddress) {
  $.ajax({
    url:
      "https://maps.googleapis.com/maps/api/geocode/json?key=AIzaSyC_uDKyICtoBmf8pI2nW0o64vS9vjWE49k&address=" +
      restAddress,
    method: "GET"
